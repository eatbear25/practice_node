...
const linePayClient = createLinePayClient({
  channelId: "2007401510",
  channelSecretKey: "63cb2c8cf67c5cfc6ce9991b1865618d",
...
router.get("/reserve", async (req, res) => {
  ...
  try {
    ...
    console.error("linePay reserve error", error);
  }
});
...
router.get("/confirm", async (req, res) => {
  ...
  if (!transactionId) {
    return console.log(res, "缺少交易編號");
  }

  if (!req.session.reservation) {
    return console.log(res, "沒有已記錄的付款資料");
  }
  ...
  try {
    ...
    console.log(res, { ...linePayResponse.body });
  } catch (error) {
...
router.get("/check-payment-status", async (req, res) => {
  ...
  try {
    ...
    console.log(res, { data: linePayResponse.body });
  } catch (error) {
    console.log(res, error);
  }
});
